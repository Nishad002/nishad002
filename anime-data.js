/* ========================================================= */
/* MAIN ANIME LIST */
/* ========================================================= */

const animeList = [

    /* ===================================================== */
    /* Anime MOVIES */
    /* ===================================================== */

        {
            title: "Into the Forest of Fireflies' Light",
            type: "movie",
            status: "completed",
            image: "images/into-the-forest-of-fireflies-light.webp",
            link: "https://myanimelist.net/anime/10408/Hotarubi_no_Mori_e"
        },

        {
            title: "I Want To Eat Your Pancreas",
            type: "movie",
            status: "completed",
            image: "images/i-want-to-eat-your-pancreas.webp",
            link: "https://myanimelist.net/anime/36098/Kimi_no_Suizou_wo_Tabetai"
        },

        {
            title: "The Tunnel to Summer, the Exit of Goodbyes",
            type: "movie",
            status: "completed",
            image: "images/the-tunnel-to-summer-the-exit-of-goodbyes.webp",
            link: "https://myanimelist.net/anime/50593/Natsu_e_no_Tunnel_Sayonara_no_Deguchi"
        },

        {
            title: "Fireworks, Should We See It from the Side or the Bottom?",
            type: "movie",
            status: "completed",
            image: "images/fireworks-should-we-see-it-from-the-side-or-the-bottom.webp",
            link: "https://myanimelist.net/anime/34498/Uchiage_Hanabi_Shita_kara_Miru_ka_Yoko_kara_Miru_ka?"
        },

        {
            title: "Hello World",
            type: "movie",
            status: "completed",
            image: "images/hello-world.webp",
            link: "https://myanimelist.net/anime/38816/Hello_World"
        },

        {
            title: "Patema Inverted",
            type: "movie",
            status: "completed",
            image: "images/patema-inverted.webp",
            link: "https://myanimelist.net/anime/12477/Sakasama_no_Patema"
        },

        {
            title: "Your Name",
            type: "movie",
            status: "completed",
            image: "images/your-name.webp",
            link: "https://myanimelist.net/anime/32281/Kimi_no_Na_wa"
        },

        {
            title: "A Silent Voice",
            type: "movie",
            status: "completed",
            image: "images/a-silent-voice.webp",
            link: "https://myanimelist.net/anime/28851/Koe_no_Katachi"
        },

        {
            title: "Her Blue Sky",
            type: "movie",
            status: "completed",
            image: "images/her-blue-sky.webp",
            link: "https://myanimelist.net/anime/39569/Sora_no_Aosa_wo_Shiru_Hito_yo"
        },

        {
            title: "Suzume",
            type: "movie",
            status: "completed",
            image: "images/suzume.webp",
            link: "https://myanimelist.net/anime/50594/Suzume_no_Tojimari"
        },

        {
            title: "Weathering With You",
            type: "movie",
            status: "completed",
            image: "images/weathering-with-you.webp",
            link: "https://myanimelist.net/anime/38826/Tenki_no_Ko"
        },

        {
            title: "Josee, The Tiger And The Fish",
            type: "movie",
            status: "completed",
            image: "images/josee-the-tiger-and-the-fish.webp",
            link: "https://myanimelist.net/anime/40787/Josee_to_Tora_to_Sakana-tachi"
        },

        {
            title: "5 Centimeters per Second",
            type: "movie",
            status: "completed",
            image: "images/5-centimeters-per-second.webp",
            link: "https://myanimelist.net/anime/1689/Byousoku_5_Centimeter"
        },

        {
            title: "Grave Of The Fireflies",
            type: "movie",
            status: "completed",
            image: "images/grave-of-the-fireflies.webp",
            link: "https://myanimelist.net/anime/578/Hotaru_no_Haka"
        },

        {
            title: "Paprika",
            type: "movie",
            status: "completed",
            image: "images/paprika.webp",
            link: "https://myanimelist.net/anime/1943/Paprika"
        },


    /* ===================================================== */
    /* Anime SERIES */
    /* ===================================================== */

        {
            title: "Hunter × Hunter",
            type: "series",
            status: "completed",
            image: "images/hunter-x-hunter.webp",
            link: "https://myanimelist.net/anime/11061/Hunter_x_Hunter_2011"
        },

        {
            title: "Death Note",
            type: "series",
            status: "completed",
            image: "images/death-note.webp",
            link: "https://myanimelist.net/anime/1535/Death_Note"
        },

        {
            title: "Steins;Gate",
            type: "series",
            status: "completed",
            image: "images/steins-gate.webp",
            link: "https://myanimelist.net/anime/9253/Steins_Gate"
        },

        {
            title: "Violet Evergarden",
            type: "series",
            status: "completed",
            image: "images/violet-evergarden.webp",
            link: "https://myanimelist.net/anime/33352/Violet_Evergarden"
        },

        {
            title: "Darling in the Franxx",
            type: "series",
            status: "completed",
            image: "images/darling-in-the-franxx.webp",
            link: "https://myanimelist.net/anime/35849/Darling_in_the_FranXX"
        },

        {
            title: "ERASED",
            type: "series",
            status: "completed",
            image: "images/erased.webp",
            link: "https://myanimelist.net/anime/31043/Boku_dake_ga_Inai_Machi"
        },

        {
            title: "Anohana: The Flower We Saw That Day",
            type: "series",
            status: "completed",
            image: "images/anohana-the-flower-we-saw-that-day.webp",
            link: "https://myanimelist.net/anime/9989/Ano_Hi_Mita_Hana_no_Namae_wo_Bokutachi_wa_Mada_Shiranai"
        },

        {
            title: "Your Lie in April",
            type: "series",
            status: "completed",
            image: "images/your-lie-in-april.webp",
            link: "https://myanimelist.net/anime/23273/Shigatsu_wa_Kimi_no_Uso"
        },

        {
            title: "Fruits Basket",
            type: "series",
            status: "completed",
            image: "images/fruits-basket.webp",
            link: "https://myanimelist.net/anime/38680/Fruits_Basket_1st_Season"
        },

        {
            title: "Attack on Titan",
            type: "series",
            status: "completed",
            image: "images/attack-on-titan.webp",
            link: "https://myanimelist.net/anime/16498/Shingeki_no_Kyojin"
        },

        {
            title: "Fullmetal Alchemist: Brotherhood",
            type: "series",
            status: "completed",
            image: "images/fullmetal-alchemist-brotherhood.webp",
            link: "https://myanimelist.net/anime/5114/Fullmetal_Alchemist__Brotherhood"
        },

        {
            title: "Tokyo Ghoul",
            type: "series",
            status: "completed",
            image: "images/tokyo-ghoul.webp",
            link: "https://myanimelist.net/anime/22319/Tokyo_Ghoul"
        },

        {
            title: "To Your Eternity",
            type: "series",
            status: "completed",
            image: "images/to-your-eternity.webp",
            link: "https://myanimelist.net/anime/41025/Fumetsu_no_Anata_e"
        },

        {
            title: "Hyouka",
            type: "series",
            status: "completed",
            image: "images/hyouka.webp",
            link: "https://myanimelist.net/anime/12189/Hyouka"
        },

        {
            title: "Horimiya",
            type: "series",
            status: "completed",
            image: "images/horimiya.webp",
            link: "https://myanimelist.net/anime/42897/Horimiya"
        },

        {
            title: "Summer Time Rendering",
            type: "series",
            status: "completed",
            image: "images/summer-time-rendering.webp",
            link: "https://myanimelist.net/anime/47194/Summertime_Render"
        },

        {
            title: "The Detective Is Already Dead",
            type: "series",
            status: "completed",
            image: "images/the-detective-is-already-dead.webp",
            link: "https://myanimelist.net/anime/46471/Tantei_wa_Mou_Shindeiru"
        },

        {
            title: "86 (Eighty-Six)",
            type: "series",
            status: "completed",
            image: "images/86-eighty-six.webp",
            link: "https://myanimelist.net/anime/41457/86"
        },

        {
            title: "Alya Sometimes Hides Her Feelings in Russian",
            type: "series",
            status: "completed",
            image: "images/alya-sometimes-hides-her-feelings-in-russian.webp",
            link: "https://myanimelist.net/anime/54744/Tokidoki_Bosotto_Russia-go_de_Dereru_Tonari_no_Aalya-san"
        },

        {
            title: "Shikimori's Not Just a Cutie",
            type: "series",
            status: "completed",
            image: "images/shikimoris-not-just-a-cutie.webp",
            link: "https://myanimelist.net/anime/45613/Kawaii_dake_ja_Nai_Shikimori-san"
        },

        {
            title: "The Angel Next Door Spoils Me Rotten",
            type: "series",
            status: "completed",
            image: "images/the-angel-next-door-spoils-me-rotten.webp",
            link: "https://myanimelist.net/anime/50739/Otonari_no_Tenshi-sama_ni_Itsunomanika_Dame_Ningen_ni_Sareteita_Ken"
        },

        {
            title: "The Promised Neverland",
            type: "series",
            status: "completed",
            image: "images/the-promised-neverland.webp",
            link: "https://myanimelist.net/anime/37779/Yakusoku_no_Neverland"
        },

        {
            title: "No Game No Life",
            type: "series",
            status: "completed",
            image: "images/no-game-no-life.webp",
            link: "https://myanimelist.net/anime/19815/No_Game_No_Life"
        },

        {
            title: "Assassination Classroom",
            type: "series",
            status: "completed",
            image: "images/assassination-classroom.webp",
            link: "https://myanimelist.net/anime/24833/Ansatsu_Kyoushitsu"
        },

        {
            title: "Noragami",
            type: "series",
            status: "completed",
            image: "images/noragami.webp",
            link: "https://myanimelist.net/anime/20507/Noragami"
        },

        {
            title: "Haikyu!!",
            type: "series",
            status: "completed",
            image: "images/haikyu.webp",
            link: "https://myanimelist.net/anime/20583/Haikyuu"
        },

        {
            title: "Another",
            type: "series",
            status: "completed",
            image: "images/another.webp",
            link: "https://myanimelist.net/anime/11111/Another"
        },

        {
            title: "The World's Finest Assassin",
            type: "series",
            status: "completed",
            image: "images/the-worlds-finest-assassin.webp",
            link: "https://myanimelist.net/anime/47790/Sekai_Saikou_no_Ansatsusha_Isekai_Kizoku_ni_Tensei_suru"
        },

        {
            title: "Rascal Does Not Dream of Bunny Girl Senpai",
            type: "series",
            status: "completed",
            image: "images/rascal-does-not-dream-of-bunny-girl-senpai.webp",
            link: "https://myanimelist.net/anime/37450/Seishun_Buta_Yarou_wa_Bunny_Girl_Senpai_no_Yume_wo_Minai"
        },

        {
            title: "Classroom of the Elite",
            type: "series",
            status: "completed",
            image: "images/classroom-of-the-elite.webp",
            link: "https://myanimelist.net/anime/35507/Youkoso_Jitsuryoku_Shijou_Shugi_no_Kyoushitsu_e"
        },

        {
            title: "Demon Slayer",
            type: "series",
            status: "completed",
            image: "images/demon-slayer.webp",
            link: "https://myanimelist.net/anime/38000/Kimetsu_no_Yaiba"
        },
            
        {
            title: "Solo Leveling",
            type: "series",
            status: "completed",
            image: "images/solo-leveling.webp",
            link: "https://myanimelist.net/anime/52299/Ore_dake_Level_Up_na_Ken"
        },

        {
            title: "Sword Art Online",
            type: "series",
            status: "completed",
            image: "images/sword-art-online.webp",
            link: "https://myanimelist.net/anime/11757/Sword_Art_Online"
        },

        {
            title: "Code Geass",
            type: "series",
            status: "completed",
            image: "images/code-geass.webp",
            link: "https://myanimelist.net/anime/1575/Code_Geass__Hangyaku_no_Lelouch"
        },

        {
            title: "Mushoku Tensei",
            type: "series",
            status: "completed",
            image: "images/mushoku-tensei.webp",
            link: "https://myanimelist.net/anime/39535/Mushoku_Tensei__Isekai_Ittara_Honki_Dasu"
        },

        {
            title: "Kaguya-sama: Love Is War",
            type: "series",
            status: "completed",
            image: "images/kaguya-sama-love-is-war.webp",
            link: "https://myanimelist.net/anime/37999/Kaguya-sama_wa_Kokurasetai__Tensai-tachi_no_Renai_Zunousen"
        },

        {
            title: "SPY×FAMILY",
            type: "series",
            status: "completed",
            image: "images/spy-family.webp",
            link: "https://myanimelist.net/anime/50265/Spy_x_Family"
        },

        {
            title: "Parasyte: The Maxim",
            type: "series",
            status: "completed",
            image: "images/parasyte-the-maxim.webp",
            link: "https://myanimelist.net/anime/22535/Kiseijuu__Sei_no_Kakuritsu"
        },

        {
            title: "Wistoria: Wand and Sword",
            type: "series",
            status: "completed",
            image: "images/wistoria-wand-and-sword.webp",
            link: "https://myanimelist.net/anime/58059/Tsue_to_Tsurugi_no_Wistoria"
        },

        {
            title: "Dr. Stone",
            type: "series",
            status: "completed",
            image: "images/dr-stone.webp",
            link: "https://myanimelist.net/anime/38691/Dr_Stone"
        },

        {
            title: "Terror In Resonance",
            type: "series",
            status: "completed",
            image: "images/terror-in-resonance.webp",
            link: "https://myanimelist.net/anime/23283/Zankyou_no_Terror"
        },

        {
            title: "Charlotte",
            type: "series",
            status: "completed",
            image: "images/charlotte.webp",
            link: "https://myanimelist.net/anime/28999/Charlotte"
        },

        {
            title: "Jujutsu Kaisen",
            type: "series",
            status: "completed",
            image: "images/jujutsu-kaisen.webp",
            link: "https://myanimelist.net/anime/40748/Jujutsu_Kaisen"
        },

        {
            title: "Death Parade",
            type: "series",
            status: "completed",
            image: "images/death-parade.webp",
            link: "https://myanimelist.net/anime/28223/Death_Parade"
        },

        {
            title: "One-Punch Man",
            type: "series",
            status: "completed",
            image: "images/one-punch-man.webp",
            link: "https://myanimelist.net/anime/30276/One_Punch_Man"
        },

        {
            title: "Chainsaw Man",
            type: "series",
            status: "completed",
            image: "images/chainsaw-man.webp",
            link: "https://myanimelist.net/anime/44511/Chainsaw_Man"
        },

        {
            title: "Mob Psycho 100",
            type: "series",
            status: "completed",
            image: "images/mob-psycho-100.webp",
            link: "https://myanimelist.net/anime/32182/Mob_Psycho_100"
        },

        {
            title: "Overlord",
            type: "series",
            status: "completed",
            image: "images/overlord.webp",
            link: "https://myanimelist.net/anime/29803/Overlord"
        },

        {
            title: "The Misfit of Demon King Academy",
            type: "series",
            status: "completed",
            image: "images/the-misfit-of-demon-king-academy.webp",
            link: "https://myanimelist.net/anime/40496/Maou_Gakuin_no_Futekigousha"
        },

        {
            title: "The Future Diary",
            type: "series",
            status: "completed",
            image: "images/the-future-diary.webp",
            link: "https://myanimelist.net/anime/10620/Mirai_Nikki"
        },

        {
            title: "The Irregular at Magic High School",
            type: "series",
            status: "completed",
            image: "images/the-irregular-at-magic-high-school.webp",
            link: "https://myanimelist.net/anime/20785/Mahouka_Koukou_no_Rettousei"
        },

        {
            title: "Blue Lock",
            type: "series",
            status: "completed",
            image: "images/blue-lock.webp",
            link: "https://myanimelist.net/anime/49596/Blue_Lock"
        },

        {
            title: "The Eminence in Shadow",
            type: "series",
            status: "completed",
            image: "images/the-eminence-in-shadow.webp",
            link: "https://myanimelist.net/anime/48316/Kage_no_Jitsuryokusha_ni_Naritakute"
        },

        {
            title: "I Got a Cheat Skill in Another World",
            type: "series",
            status: "completed",
            image: "images/i-got-a-cheat-skill-in-another-world.webp",
            link: "https://myanimelist.net/anime/52830/Isekai_de_Cheat_Skill_wo_Te_ni_Shita_Ore_wa_Genjitsu_Sekai_wo_mo_Musou_suru"
        },

        {
            title: "Am I Actually the Strongest?",
            type: "series",
            status: "completed",
            image: "images/am-i-actually-the-strongest.webp",
            link: "https://myanimelist.net/anime/52969/Jitsu_wa_Ore_Saikyou_deshita"
        },

        {
            title: "Failure Frame",
            type: "series",
            status: "completed",
            image: "images/failure-frame.webp",
            link: "https://myanimelist.net/anime/57892/Hazurewaku_no_Joutai_Ijou_Skill_de_Saikyou_ni_Natta_Ore_ga_Subete_wo_Juurin_suru_made"
        },

        {
            title: "The Aristocrat's Otherworldly Adventure",
            type: "series",
            status: "completed",
            image: "images/the-aristocrats-otherworldly-adventure.webp",
            link: "https://myanimelist.net/anime/52608/Tensei_Kizoku_no_Isekai_Boukenroku"
        },

        {
            title: "The Strongest Sage with the Weakest Crest",
            type: "series",
            status: "completed",
            image: "images/the-strongest-sage-with-the-weakest-crest.webp",
            link: "https://myanimelist.net/anime/47161/Shikkakumon_no_Saikyou_Kenja"
        },

        {
            title: "The Strongest Magician in Demon Lord's Army was a Human",
            type: "series",
            status: "completed",
            image: "images/the-strongest-magician-in-demon-lord's-army-was-a-human.webp",
            link: "https://myanimelist.net/anime/57876/Maougun_Saikyou_no_Majutsushi_wa_Ningen_datta"
        },

        {
            title: "In Another World with My Smartphone",
            type: "series",
            status: "completed",
            image: "images/in-another-world-with-my-smartphone.webp",
            link: "https://myanimelist.net/anime/35203/Isekai_wa_Smartphone_to_Tomo_ni"
        },

        {
            title: "The Reincarnation of the Strongest Exorcist",
            type: "series",
            status: "completed",
            image: "images/the-reincarnation-of-the-strongest-exorcist.webp",
            link: "https://myanimelist.net/anime/50932/Saikyou_Onmyouji_no_Isekai_Tenseiki"
        },

        {
            title: "I Was Reincarnated As The 7th Prince",
            type: "series",
            status: "completed",
            image: "images/i-was-reincarnated-as-the-7th-prince.webp",
            link: "https://myanimelist.net/anime/53516/Tensei_shitara_Dainana_Ouji_Datta_node_Kimama_ni_Majutsu_wo_Kiwamemasu"
        },
        
        {
            title: "Wise Man's Grandchild",
            type: "series",
            status: "completed",
            image: "images/wise-mans-grandchild.webp",
            link: "https://myanimelist.net/anime/36407/Kenja_no_Mago"
        },

        {
            title: "May I Ask for One Final Thing?",
            type: "series",
            status: "completed",
            image: "images/may-i-ask-for-one-final-thing.webp",
            link: "https://myanimelist.net/anime/59846/Saigo_ni_Hitotsu_dake_Onegai_shitemo_Yoroshii_deshou_ka"
        },

        {
            title: "The Wrong Way to Use Healing Magic",
            type: "series",
            status: "completed",
            image: "images/the-wrong-way-to-use-healing-magic.webp",
            link: "https://myanimelist.net/anime/49613/Chiyu_Mahou_no_Machigatta_Tsukaikata"
        },

        {
            title: "The Brilliant Healer's New Life in the Shadows",
            type: "series",
            status: "completed",
            image: "images/the-brilliant-healer's-new-life-in-the-shadows.webp",
            link: "https://myanimelist.net/anime/58359/Isshun_de_Chiryou_shiteita_noni_Yakutatazu_to_Tsuihou_sareta_Tensai_Chiyushi_Yami_Healer_toshite_Tanoshiku_Ikiru"
        },

        {
            title: "Easygoing Territory Defense",
            type: "series",
            status: "completed",
            image: "images/easygoing-territory-defense.webp",
            link: "https://myanimelist.net/anime/61549/Okiraku_Ryoushu_no_Tanoshii_Ryouchi_Bouei__Seisankei_Majutsu_de_Na_mo_Naki_Mura_wo_Saikyou_no_Jousai_Toshi_ni"
        },

        {
            title: "Reborn to Master the Blade",
            type: "series",
            status: "completed",
            image: "images/reborn-to-master-the-blade.webp",
            link: "https://myanimelist.net/anime/50481/Eiyuuou_Bu_wo_Kiwameru_Tame_Tenseisu"
        },

        {
            title: "Jack of All Trades, Party of None",
            type: "series",
            status: "completed",
            image: "images/jack-of-all-trades-party-of-none.webp",
            link: "https://myanimelist.net/anime/61128/Yuusha_Party_wo_Oidasareta_Kiyoubinbou"
        },

        {
            title: "Lookism",
            type: "series",
            status: "completed",
            image: "images/lookism.webp",
            link: "https://myanimelist.net/anime/53149/Lookism"
        },

        {
            title: "Ninja Kamui",
            type: "series",
            status: "completed",
            image: "images/ninja-kamui.webp",
            link: "https://myanimelist.net/anime/56285/Ninja_Kamui"
        },


];